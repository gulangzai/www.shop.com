...
public class RoleController {
	...
	@Autowired
	@Qualifier("sjRoleServiceImpl")
	SjRoleService sjRoleService;
	...
	public requestJson queryRole(HttpServletRequest request, requestJson json) {
		...
		try {
			String domResult = sjRoleService.queryRole(json.getMsg(), user);
			j.setMsg(domResult);
	...
	protected requestJson executeRole(HttpServletRequest request,requestJson json) throws Exception {
		...
		try {
			String domResult = sjRoleService.executeRole(json.getMsg(), user, operatorSign);
			j.setMsg(domResult);
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    protected requestJson delete(HttpServletRequest request, requestJson json) throws Exception {
       // User user = RestContext.getCurrentUser();
        ...
        boolean flag = this.sjRoleService.delete(role_uid);
        j.setSuccess(flag);
        return j;
    }
