 ...
 * 
 */
public class ProjectMenuTopTag extends TagSupport {
	...
	public String getMenuTreeHtml() throws Exception {
		...
		if (menus != null && menus.length > 0) {
			for (int i = 0; i < menus.length; i++) {
				...
				menuTreeHtml.append(" <a id=\"top-"+menus[i].getAuthorityCode()+"\" href=\"javascript:void(0);\" onclick=\"doPage('"+menus[i].getUrl()+"','top-"+menus[i].getAuthorityCode()+"','"+menus[i].getAuthorityUid()+"','"+menus[i].getAuthorityUid()+"');\"> ");
	...
	protected AuthorityVo[] getAllowedMenus(User user, String parent) throws Exception {
		...
		//return ProjectMenuManager.getInstance().getMenusTop(user);
