					   ...
					   role="form" style="border:solid 1px #ddd;height:50px;line-height:50px;vertical-align:middle;">
					    ...
					    <table  sortname="BIANGENG_TYPE" multiselect=false  rownum="1000000" sortorder="asc"  class=""  id="grid_table" 
							...
							<tr>
								...
								<th name="BIANGENG_DATE" width="10" align="center">签证日期</th>
								<!-- <th name="BIANGENG_TYPE" width="10" align="center" formatter="formatType">签证类型</th> -->
								<th name="REASON" width="30" align="left" formatter="shiyou">签证事由</th>
								<th name="BIANGENG_COMPANY" width="20" align="center" formatter="fordanwei">签证发起单位</th>
								<th name="AMOUNT" width="10" align="right">签证金额</th>
...
function shiyou(cellvalue, opts, rowObject){
	  ...
	  var  thtml = '<div class="pull-left">';
        if(reason.length<30){
	    	  ...
	    	  thtml += '<span class="bigger-100">'+reason.substring(0,28)+'...</span>';
	      }
...
function init(){	
	...
	$('#newAdd').attr("data-target","jihbg-input");
		...
		$.get("${ctx}/jsp/business/qizbggl/qianzgl-add.jsp",function(data) {
			$("#jihbg-input").empty();
			$("#jihbg-input").html("");
...
function eidtBiangeng(bid){
		...
		$.get("${ctx}/jsp/business/qizbggl/qianzgl-update.jsp?bid="+bid,function(data) {
			$("#jihbg-input2").empty();
			$("#jihbg-input2").html("");
...
function showeidtBiangeng(bid){
	...
	$.get("${ctx}/jsp/business/qizbggl/qianzgl-show.jsp?bid="+bid,function(data) {
		$("#jihbg-input2").empty();
		$("#jihbg-input2").html("");
		$("#jihbg-input2").html(data);
	});
