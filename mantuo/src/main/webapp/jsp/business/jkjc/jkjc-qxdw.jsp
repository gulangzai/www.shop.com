...
<body >
	<div class="main-container" id="main-container">
		    ...
		    <!-- form 开始 位置 -->
					<form class="form-inline"  id="queryForm04" width="100%"
				   ...
				   <form id="dataCheckBox04">
				      <!-- 当前 选择的 checkbox -->
				       <input class="hidden" id="strUid" type="hidden" fieldname="PERSON_INFO_UID"/>
				    </form>
					    <table   sortname="PERSON_INFO_UID" multiselect=false  rownum="1000000" sortorder="asc"  class=""  id="grid_table04" 
...
ace.load_ajax_scripts(scripts, function() {
	...
	JqgridManage.initJqgrid(grid_table04,queryForm04,gridwidth); 
	...
	if(navigator.userAgent.indexOf("Firefox") > -1){
	 ...
	 $("#grid_table04").setGridHeight(setHeight - 320); 
});

//enter键
function reSearch(){
$("#searchBox").click(function(){
	  jQuery("#grid_table04").jqGrid().trigger("reloadGrid");
	}); 
...
function init(){	
	...
	$('#newDel').click(function (){
		//获取 选择的列
		   var v="";
		   $("#grid_table04 input:checkbox[name='PERSON_INFO_UID']:checked").each(function() {
				v += $(this).val() + "," ;
			});
			if (v.length != 0) {
		  ...
		  if($("#strUid").val() !="" ){
			bootbox.confirm("确认删除吗？", function(result) {
				if(result){
				    var data = Form2Json.formToJSON(dataCheckBox04);
					//组成保存json串格式
...
function eidtPerson(pid){
	...
	$('#newAdd').attr("data-target","jkcjlxr-input");
		...
		$.get("${ctx}/jsp/business/jkjc/jkjcJcqxdw-edity.jsp?PROJECT_UID="+$("#project_uid").val()+"&JCUID="+pid,function(data) {
			$("#jkcjlxr-input").empty();
			$("#jkcjlxr-input").html("");
