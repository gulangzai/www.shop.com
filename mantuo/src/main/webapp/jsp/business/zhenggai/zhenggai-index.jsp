					...
					<form class="form-inline"  id="queryForm" width="100%"
					   role="form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
						 <input id="PROJECT_UID" type="hidden" name="PROJECT_UID" fieldname="ZG.PROJECT_UID" operation="="/> 
						  ...
						  <input id="BIANGENG_TYPE_FORSELECT"  type="hidden"  fieldname="ZG_TYPE" operation="in" value="" fieldtype="text"/> 
					    ...
					    <table  sortname="ZG.ZG_DATE"   rownum="1000000"  sortorder="desc"  class="" id="grid_table" 
									action="${ctx}/zhenggai/pmZhengGaiController?query" >
							<tr>
								<%--<th name="XIANCHANG_STRUC_UID"  align="center" width="0"></th>
								--%> 
								<th name="STATUS"   align="center" width="2" formatter="showStatus">整改状态</th>
								<th name="ZG_CODE"  align="center" width="3" formatter="" >整改编号</th>
								<th name="ZG_TYPE"  align="center" width="3" formatter="doType" >整改类型</th>
								<th name="SGDW"  align="center" width="3" formatter="showsgdw" >施工单位</th>
								<th name="FAFANG_DATE"  align="center" width="3" formatter="" >发放日期</th>
								<th name="FAFANG_CORP"  align="center" width="3" formatter="showfafangcorp" >发放单位</th>
								<th name="FAFANG_USER"  align="center" width="3" formatter="" >发放人</th>
								<th name="ZG_DATE"  align="center" width="3" formatter="showZgDate" >要求完成日期</th>
								  
								<th name="XIANCHANG_UID"  align="center" width="2" formatter="doUpdate">编辑</th>
								<th name="XIANCHANG_UID" align="center" width="4" formatter="doAnswerAsk">操作</th>
							</tr> 
						</table> 
...
<script type="text/javascript">
var controlletname="${pageContext.request.contextPath }/zhenggai/pmZhengGaiController";
var scripts =[null];
var gridwidth;
var setHeight;
...
function init(){	
	...
	$('#newAdd').attr("data-target","xmgk-input");
		...
		$.get("${ctx}/jsp/business/zhenggai/zhenggai-add.jsp",function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
...
// 查看按钮不可修改
function showFileList(project_uid,zhenggai_uid,zg_type){
	//alert("暂时不开放");
    $('#newAdd').attr("data-target","xmgk-input");
		$('#xmgk-input').removeData("bs.modal");
		$("#xmgk-input").empty();
		...
		$.get("${ctx}/jsp/business/zhenggai/zhenggai-detail.jsp?PROJECT_UID="+project_uid+"&ZHENGGAI_UID="+zhenggai_uid+"&ZG_TYPE="+zg_type,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
...
//查看信息
function eidtXcZlxx(project_uid,zhenggai_uid,zg_type){
		$('#newAdd').attr("data-target","xmgk-input");
		$('#xmgk-input').removeData("bs.modal");
		$("#xmgk-input").empty();
		$('#xmgk-input').modal({
		...
		$.get("${ctx}/jsp/business/zhenggai/zhenggai-edit.jsp?PROJECT_UID="+project_uid+"&ZHENGGAI_UID="+zhenggai_uid+"&ZG_TYPE="+zg_type,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
...
function doUpdate(cellvalue, opts, rowObject){
	...
	var showHtml ="<div class='hidden-sm hidden-xs action-buttons' style='padding-top:15px;'>";	
		...
		showHtml +="onclick=\"showFileList('"+project_uid+"','"+zhenggai_uid+"','"+ZG_TYPE+"');\">" ;
		...
		showHtml +=	" onclick=\"eidtXcZlxx('"+project_uid+"','"+zhenggai_uid+"','"+ZG_TYPE+"');\">";
		...
		showHtml +=	" onclick=\"removeData('"+project_uid+"','"+zhenggai_uid+"');\">";
...
function doAnswerAsk(cellvalue, opts, rowObject){
   ...
   if(checkTabAuthority($("#project_uid").val(),$("#projectUserId").val(),"12080004")){
		showHtml = '<div style="padding:15px;"><a href="javascript:if('+STATUS+'==-1){alert(\'草稿状态不能进行答复\');}else{openDf('+ZHENGGAI_UID+');}">答复</a>&nbsp<div>';
   }
		return 	showHtml;
}

//打开答复列表
function openDf(ZHENGGAI_UID){  
