	...
	<div class="main-container" id="main-container">
		    ...
		    <!-- form 开始 位置 -->
					<form class="form-inline"  id="queryForm02" width="100%"
					   role="form" style="border:solid 1px #ddd;height:50px;line-height:50px;vertical-align:middle;">
						...
						<div style="float: right;">
							 <div   class="form-group" >
								<a data-target="" id="newAdd02" data-toggle="modal">
									<button id="btnAdd02" class="btn btn-link btn-sm" type="button"><i class="ace-icon glyphicon glyphicon-plus"></i><span style="font-family:'Microsoft YaHei';font-size:15px;color:#4e4c4c;">添加</span></button>
								</a>
							 </div>
							 &nbsp;
							<!--  <button class="btn btn-white btn-default btn-round"><i class="ace-icon fa fa-times red2"></i>删除</button> -->
							 <div class="form-group" >
								<a data-target="" id="newDel02" data-toggle="modal">
									<button id="btnDel02" class="btn btn-link btn-sm" type="button"><i class="ace-icon glyphicon glyphicon-remove red bigger-110"></i><span style="font-family:'Microsoft YaHei';font-size:15px;color:#4e4c4c;">删除</span></button>
								</a>
							 </div> 
							 &nbsp; 
							<div class="form-group">
				   ...
				   <form id="dataCheckBox02">
				      <!-- 当前 选择的 checkbox -->
				       <input class="hidden" id="strUid02" type="hidden" fieldname="PERSON_INFO_UID"/>
				    </form>
					    <table   sortname="PERSON_INFO_UID" multiselect=false  rownum="1000000" sortorder="asc"  class=""  id="grid_table02" 
		...
		<div id="jsjc-fzrllfs-input" class="modal"></div>
...
function init(){	
	...
	$('#newAdd02').attr("data-target","jsjc-fzrllfs-input");
		$('#jsjc-fzrllfs-input').removeData("bs.modal");
		$("#jsjc-fzrllfs-input").empty();
		$('#jsjc-fzrllfs-input').modal({
			backdrop: 'static'
		});
       
		$.get("${ctx}/jsp/business/jsjc/jsjc-fzrllfs-add.jsp?PROJECT_UID="+$("#project_uid").val(),function(data) {
			$("#jsjc-fzrllfs-input").empty();
			$("#jsjc-fzrllfs-input").html("");
			$("#jsjc-fzrllfs-input").html(data);
		});
	
	});
	
	//删除 
	$('#btnDel02').click(function (){
		//获取 选择的列
		   var v="";
		   $("#grid_table02 input:checkbox[name='PERSON_INFO_UID']:checked").each(function() {
				v += $(this).val() + "," ;
			});
			if (v.length != 0) {
