					...
					<form class="form-inline"  id="queryForm" width="100%"
					   role="form" style="border:solid 1px #ddd;height:50px;line-height:50px;vertical-align:middle;">
						 <input id="PROJECT_UID" type="hidden" name="p.PROJECT_UID" fieldname="p.PROJECT_UID" operation="="/> 
						  <input id="" type="hidden" fieldname="xc.GONGKUANG_TYPE" operation="=" value="AQ"/> 
...
function init(){	
	...
	$('#newAdd').attr("data-target","xmgk-input");
		...
		$.get("${ctx}/jsp/business/xmzlaq/xcqk-aqorganize-add.jsp",function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
...
function showFilesOrImgs(xc_uid){
	...
	$('#newAdd').attr("data-target","xmgk-input");
		...
		$.get("${ctx}/jsp/business/xmzlaq/xczk-showfilesorimgs.jsp?XC_UID="+xc_uid+"&TYPE=10011",function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
	...
	var showHtml ="<div class='hidden-sm hidden-xs action-buttons' style='padding-top:15px;'>";	
		...
		if(checkTabAuthority($("#project_uid").val(),$("#projectUserId").val(),"12060102")){
			if(status=="0"||status=="-1"){	
				...
				showHtml +=	" onclick=\"editXcAqxx('"+xc_uid+"','"+xs_uid+"');\">";
...
function showFileList(xc_uid,xs_uid){
		...
		$.get("${ctx}/jsp/business/xmzlaq/xczk-onlyrivew-aqdetail.jsp?XC_UID="+xc_uid+"&XS_UID="+xs_uid,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
...
function editXcAqxx(xc_uid,xs_uid){
		$('#newAdd').attr("data-target","xmgk-input");
		$('#xmgk-input').removeData("bs.modal");
		$("#xmgk-input").empty();
		$('#xmgk-input').modal({
		...
		$.get("${ctx}/jsp/business/xmzlaq/xczk-aqorganize-update.jsp?XC_UID="+xc_uid+"&XS_UID="+xs_uid,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
...
function doAnswerAsk(cellvalue, opts, rowObject){
    ...
    var showHtml ="<div class='hidden-sm hidden-xs action-buttons' style='padding-top:15px;'>";			        	
		...
		showHtml +=	"onclick=\"answerXcAq('"+xc_uid+"');\">";
...
function answerXcAq(xc_uid){
	$('#newAdd').attr("data-target","xmgk-input");
		...
		$.get("${ctx}/jsp/business/xmzlaq/xczk-jdorganize-answer.jsp?XC_UID="+xc_uid+"&FILE_TYPE=10011",function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
