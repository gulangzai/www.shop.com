...
<div class="modal-dialog width-60 height-auto">
  <div class="modal-content" >
   <div class="widget-header widget-header-large">
    	...
    	<h3 id="myModalLabel" class="blue bigger">添加安全工况</h3>
  </div>
  <div class="modal-body">
 	  <form method="post" role="form" class="form-horizontal"  id="executeFrmXcAQ" > 
 	   <!-- 获取 项目 的 uid -->    	
      	 <input id="XCROJECT_UID" type="hidden" name="ROJECT_UID" fieldname="PROJECT_UID"  />
			...
			-->	
				...
				<div class="col-sm-4">
				 <input  id="FINISH_DATE" style="width: 80%;" class="col-xs-11 col-sm-11" data-date-format="yyyy-mm-dd"  datatype="*" nullmsg="请选择要求解决日期"   nullmsg="请选择要求解决日期"  placeholder="请选择要求解决日期" type="text"  fieldname="FINISH_DATE" />
				</div>
				<label class="col-sm-2 control-label no-padding-right" >
		    ...
		    <div class="form-group">
					...
					<div class="col-sm-9">
						...
						<table  role="presentation" class="table table-striped">
							<tbody class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery"  file_type="10011" fjlb="media"></tbody>
						</table>
					</div>
		   </div>
     </form>
  ...
  <button id="addXcAq" class="btn btn-1 btn-sms" type="button">发布</button>
  <button id="baocun" class="btn btn-1 btn-sms" type="button">&nbsp;保存&nbsp;</button>
    
    <button id="closeAq"  class="btn btn-2 btn-sms pull-right" data-dismiss="modal" aria-hidden="true">关闭</button>
  </div>
</div>
</div>
...
$(function() {
	...
	validform=FormValid.validbyformid(executeFrmXcAQ);
	$('#XCROJECT_UID').val($('#project_uid').val());
	$("#addXcAq").click(function() {
		$("#STATUS").val("0");
		var date =$("#FINISH_DATE").val();
		var d1 = new Date(date.replace(/\-/g, "\/")); 
		 var d2 = new Date(dates.replace(/\-/g, "\/"));
	  if(validform.check()&&d1>=d2){
		if($("#executeFrmXcAQ").validationButton()) {
			var data = Form2Json.formToJSON(executeFrmXcAQ);
