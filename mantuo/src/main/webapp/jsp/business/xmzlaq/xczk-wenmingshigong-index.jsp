						...
						<div style="float: right;">
							 <div id="btnAddDiv"  class="form-group" >
								<a data-target="" id="newAdd" data-toggle="modal">
									<button style="display: none;" id="btn_new" class="btn btn-link btn-sm" type="button"><i class="colorshow ace-icon glyphicon glyphicon-plus"></i>
...
ace.load_ajax_scripts(scripts, function() {
    ...
    $(".footer-content").remove();//去除页脚下的版本符号
     ...
     DatePicker.datepicker("#REPORT_DATE_ONE1"); 
	//获取 当前的 项目的 主键 uid
...
function init(){	
	...
	$("#searchForPlan").click(function(){
		//alert($("#grid_table").attr("action"));
		
		var date= $("#REPORT_DATE11").val();
		if(date !=""){
			$("#REPORT_DATE11").val(date+" 23:59:59");
		}
		//$("#grid_table").attr("action","${ctx}/pmxianchang/pmXianchangController?query&date1="+date1+"&date2="+date2+"");
	 	//alert($("#grid_table").attr("action"));
	...
	$('#newAdd').attr("data-target","xmgk-input");
		...
		$.get("${ctx}/jsp/business/xmzlaq/xczk-wenmingshigong-add.jsp",function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
...
function showFileList(xc_uid,xs_uid){
		...
		$.get("${ctx}/jsp/business/xmzlaq/xczk-onlyriview-wmdetail.jsp?XC_UID="+xc_uid+"&XS_UID="+xs_uid,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
...
function eidtXcZlxx(xc_uid,xs_uid){
		...
		$.get("${ctx}/jsp/business/xmzlaq/xczk-wenmingshigong-update.jsp?XC_UID="+xc_uid+"&XS_UID="+xs_uid,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
