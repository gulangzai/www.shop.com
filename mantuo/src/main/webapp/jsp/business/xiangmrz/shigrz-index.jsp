		    ...
		    <!-- form 开始 位置 -->
					<form class="form-inline"  id="queryForm" width="100%"
					   role="form" style="border:solid 1px #ddd;height:50px;line-height:50px;vertical-align:middle;">
						 <input id="PROJECT_UID" type="hidden" name="l.PROJECT_UID" fieldname="l.PROJECT_UID" operation="="/> 
						 <input  type="hidden"  fieldname="l.LOG_TYPE" operation="=" value="SG"/> 
						<div class="form-group"><!-- style="width: 130px; height: 25px;border-radius: 8px 8px 8px 8px; border: 1px solid #d1d2d9;" -->
						 ...
						 <input class="form-control" data-date-format="yyyy-mm-dd"
								style="width: 130px; height: 25px;border-radius: 8px 8px 8px 8px; border: 1px solid #d1d2d9;"		
										fieldname="l.LOG_DATE" id="REPORT_DATE1"
										operation="<=" logic="and" />
										&nbsp;&nbsp; 
						<!-- 				
						    单位工程名称：				
						 <input id="selectId" placeholder="单位工程名称" class="form-control" onclick="select();" style="width: 150px; height: 25px;border-radius: 8px 8px 8px 8px; border: 1px solid #d1d2d9;
					    ...
					    <table   sortname="l.PROJECT_LOG_UID" multiselect=false  rownum="1000000" sortorder="asc"  class=""  id="grid_table" 
							...
							<tr>
								...
								<th name="SHENGCHAN_INFO" width="8"  align="left">生产情况记录</th>
								<th name="TUFA" width="8" align="left">突发事件</th>
								<th name="USER_NAME" width="2" align="center">填写人</th>
								
								<th name="PROJECT_LOG_UID" width="2" align="center" formatter="doUpdate">操作</th>
							</tr> 
						</table> 
...
ace.load_ajax_scripts(scripts, function() {
	...
	//权限检查
	checkBtnAuthority($("#project_uid").val(),$("#projectUserId").val(),"12060101","btn_new"); 
});
...
function init(){	
	...
	$('#newAdd').attr("data-target","xmgk-input");
		...
		$.get("${ctx}/jsp/business/xiangmrz/shigrz-add.jsp",function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
...
function eidtLlog(log_uid){
		...
		$.get("${ctx}/jsp/business/xiangmrz/shigrz-update.jsp?log_uid="+log_uid,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
...
function showeidtLlog(log_uid){
	...
	$.get("${ctx}/jsp/business/xiangmrz/shigrz-show.jsp?log_uid="+log_uid,function(data) {
		$("#xmgk-input").empty();
		$("#xmgk-input").html("");
		$("#xmgk-input").html(data);
	});
