		    ...
		    <!-- form 开始 位置 -->
					<form class="form-inline"  id="queryForm" width="100%"
					   role="form" style="border:solid 1px #ddd;height:50px;line-height:50px;vertical-align:middle;">
						 ...
						 <input  type="hidden" fieldname="l.LOG_TYPE" operation="=" value="JL"/> 
					    ...
					    <table   sortname="l.PROJECT_LOG_UID" multiselect=false  rownum="1000000" sortorder="asc"  class=""  id="grid_table" 
							...
							<tr>
								...
								<th name="GONGZUO_INFO" width="8"  align="left">主要工作情况</th>
								<th name="TUFA"  align="left" width="8">突发事件</th>
								<th name="USER_NAME"  align="center" width="2">填写人</th>
...
function init(){	
	...
	$('#newAdd').attr("data-target","xmgk-input");
		...
		$.get("${ctx}/jsp/business/xiangmrz/jianlrz-add.jsp",function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
...
function eidtLlog(log_uid){
		...
		$.get("${ctx}/jsp/business/xiangmrz/jianlrz-update.jsp?log_uid="+log_uid,function(data) {
			$("#xmgk-input").empty();
			$("#xmgk-input").html("");
			$("#xmgk-input").html(data);
		});
...
function showeidtLlog(log_uid){
	...
	$.get("${ctx}/jsp/business/xiangmrz/jianlrz-show.jsp?log_uid="+log_uid,function(data) {
		$("#xmgk-input").empty();
		$("#xmgk-input").html("");
		$("#xmgk-input").html(data);
	});
