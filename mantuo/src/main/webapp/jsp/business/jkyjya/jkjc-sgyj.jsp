...
<body >
	<div class="main-container" id="main-container">
		    ...
		    <!-- form 开始 位置 -->
					<form class="form-inline"  id="queryForm03" width="100%"
					   role="form" style="border:solid 1px #ddd;height:50px;line-height:50px;vertical-align:middle;">
						 <input id="PLAN_TYPE" type="hidden" name="t.PLAN_TYPE" fieldname="t.PLAN_TYPE" operation="=" value="JK"/>
						 <input id="PROJECT_UID" type="hidden" name="t.PROJECT_UID" fieldname="t.PROJECT_UID" operation="="/> 
						<div class="form-group">
						  <span id="getSelect" style="font-size:15px;line-height: 50px;">&nbsp;施工应急预案</span>
						</div>
				   ...
				   <form id="dataCheckBox03">
				      <!-- 当前 选择的 checkbox -->
				       <input class="hidden" id="strUid" type="hidden" fieldname="EMERGENCY_PLAN_UID"/>
				    </form>
					    <table   sortname="EMERGENCY_PLAN_UID" multiselect=false  rownum="1000000" sortorder="asc"  class=""  id="grid_table01" 
		...
		<div id="jkjcYjya-input" class="modal"></div>
...
ace.load_ajax_scripts(scripts, function() {
	...
	JqgridManage.initJqgrid(grid_table01,queryForm03,gridwidth); 
	...
	if(navigator.userAgent.indexOf("Firefox") > -1){
	 ...
	 $("#grid_table01").setGridHeight(setHeight - 320); 
});

function init(){	
    //查询按钮 (报表)
	$("#searchForPlan").click(function(){
	  jQuery("#grid_table01").jqGrid().trigger("reloadGrid");
	}); 
	
	
	$('#new').click(function() {
	$('#new').attr("data-target","jkjcYjya-input");
		$('#jkjcYjya-input').removeData("bs.modal");
		$("#jkjcYjya-input").empty();
		$('#jkjcYjya-input').modal({
			backdrop: 'static'
		});
       ...
       /*  var xuanze = $("#ClickType").val();
		...
		$.get("${ctx}/jsp/business/jkjc/jkjcJcya-add.jsp?PROJECT_UID="+$("#project_uid").val(),function(data) {
			$("#jkjcYjya-input").empty();
			$("#jkjcYjya-input").html("");
			$("#jkjcYjya-input").html(data);
		});
	...
	$('#newDel').click(function (){
		//获取 选择的列
		   var v="";
		   $("#grid_table01 input:checkbox[name='EMERGENCY_PLAN_UID']:checked").each(function() {
				v += $(this).val() + "," ;
			});
			if (v.length != 0) {
...
$('#new').attr("data-target","jkjcYjya-input");
	...
	$.get("${ctx}/jsp/business/jkjc/jkjcJcya-edit.jsp?PROJECT_UID="+$("#project_uid").val()+"&JCUID="+yjid,function(data) {
		$("#jkjcYjya-input").empty();
		$("#jkjcYjya-input").html("");
		$("#jkjcYjya-input").html(data);
	});
