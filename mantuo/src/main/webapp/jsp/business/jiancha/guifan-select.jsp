					 	   ...
					 	   <form method="post"  class="form-horizontal"  id="queryForm">
								...
								<table  sortname="t.BZGF_UID" multiselect=false  rownum="10000" sortorder="ASC" class="auto_startgrid" id="grid_table" 
									...
									<tr>
										...
										<th name="NODE_CONTENT"  width="12" align="center">内容</th>
...
function getSettings(){
	 var setting = {
				async: {
					enable: true,
					url: controllername+"?creatTree2&project_uid="+project_uid							
				},
				view: {
...
function zTreeOnClick (event, treeId, treeNode){
	...
	//给表格添加查询条件
	if(treeNode.id==""||treeNode.NODE_TYPE=='GF'){
		$('#P_BZGF_UID').attr("operation","=");
		$('#P_BZGF_UID').val('null');
	}else{
		$('#P_BZGF_UID').attr("operation","=");
...
function init(){
	...
	$("#finishBtn").click(function(){
		var str = getRadioV("guifan");
		if(str==''||str==undefined){
			alert('请选择一条内容!');
		}else{
			if(window.opener){
				//执行父窗口方法
				window.opener.doCallback(str);
			}
			window.close();
		}
	});
...
function formatSelect(cellvalue, opts, rowObject){
	...
	if(_guifan_uid==guifan_uid){
		showHtml +="<input type=\"radio\"  name=\"guifan\" checked=\"checked\" value ='"+v+"'>";
	}else{
		showHtml +="<input type=\"radio\"  name=\"guifan\" value ='"+v+"' >";
	}			        	
