	...
	<div class="main-container" id="main-container">
		    ...
		    <!-- form 开始 位置 -->
					<form class="form-inline"  id="queryForm" width="100%"
					   ...
					   <input type="hidden"  fieldname='x.XUNJIAN_TYPE' fieldtype='text' operation='=' value="AJ">
...
ace.load_ajax_scripts(scripts, function() {
	...
	//改变浏览器大小自适应
		...
		//权限检查
		checkBtnAuthority($("#project_uid").val(),$("#projectUserId").val(),"12050501","btn_new"); 
});
...
function operate (cellvalue, opts, rowObject){
	...
	returnHTML +=	"</a> ";	
	if(checkTabAuthority($("#project_uid").val(),$("#projectUserId").val(),"12050502")){
	...
	if(checkTabAuthority($("#project_uid").val(),$("#projectUserId").val(),"12050503")){
...
function updateXx(id){
	...
	$.get("${ctx}/jsp/business/jiancha/xunjian-aj-update.jsp",{
		"xunjian_uid":id
		},function(data){
		$("#xunjian-input").empty();
		$("#xunjian-input").html("");
...
function seeXx(id){
	...
	$.get("${ctx}/jsp/business/jiancha/xunjian-aj-view.jsp",{
		"xunjian_uid":id
		},function(data){
		$("#xunjian-input").empty();
		$("#xunjian-input").html("");
...
function goAdd(){
	...
	$.get("${ctx}/jsp/business/jiancha/xunjian-aj-add.jsp",
		{
