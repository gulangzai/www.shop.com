...
<div style="display:none;" class="myKeyValueDiv">
	...
	<input type="hidden" cond="true" condName="create_user" value="<%=userid %>">
...
function insertFileTab(result){
	...
	$(".showFileTab").each(function(i){
		...
		//alert($(this).attr("showType")+"sssss") 未定义
...
function insertDetailTab(obj,files,tab_width){
	...
	for(var i=0;i<files.length;i++){
		...
		if(file_type==undefined || file_type==file.FILE_TYPE){
			showHtml +="<tr class='template-download'  style='border:1px solid #f5f5f5;' rowid='"+file.FILE_UID+"'>";
			showHtml +="<td class='name' style='border-bottom:1px solid #f5f5f5;'>";
			if(file.FILE_TYPE_NAME.length>30){
			   showHtml +="<span class='my-showPreview-btn nameSpan' style='cursor:hand' fileid='"+file.FILE_UID+"'><abbr title='"+file.FILE_TYPE_NAME+"'>"+file.FILE_TYPE_NAME.substring(0,g_nameMaxlength)+"...</abbr></span>"
			}else{
			   showHtml +="<span class='my-showPreview-btn nameSpan' style='cursor:hand' fileid='"+file.FILE_UID+"' target='_blank'>"+file.FILE_TYPE_NAME+"</span>";
			}
			showHtml +="</td>";
			
			if(obj.attr("showSize")!="false"){
				showHtml +="<td class='size ' style='border-bottom:1px solid #f5f5f5;' >"+file.FILE_SIZE/1000+"KB</td>";
			}
			...
			showHtml +="<td class='delete' style='border-bottom:1px solid #f5f5f5;'>";
		...
		if(file_type==undefined || file_type==file.FILE_TYPE){
			if(file.fileType.indexOf("image")!=-1){
				...
				if(_attr=="multi"){
					//多张图片就不需要删除了
					if(_noborder=="true"){
						showHtml +="<table class='fu_blockTable_half' cellspacing='0' cellpadding='0' border='1' style='text-align:center;'>";
					}else{
				...
				if(_preview=="true"){
					...
					//console.log(_width+"sss"+_height)
				}else{
