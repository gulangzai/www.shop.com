...
function  insertimageDetailTabtab(obj,files,tab_width){
	...
	for(var i=0;i<files.length;i++){
		...
		if(filee[1]=="jpg"||filee[1]=="gif"||filee[1]=="png"||filee[1]=="bmp"||filee[1]=="tif"){
			//如果table没有附件类别，那么插入所有数据，否则只插入附件类别相匹配的数据
			if(file_type==undefined || file_type==file.FILE_TYPE){
				if(file.MIME_TYPE.indexOf("image")!=-1){
					imgNum++;
					showHtml +="<div  class='template-download' style='"+_width+_heightss+"float:left;'>";
					if(_preview=="true"){
						...
						if(_quanxian=="true"){
						showHtml +="<div   style='"+_width+_height+"float:left;'>";
						showHtml +="<a class='lightbox'  href='<%=root%>"+file.FILE_PATH+"'><img  src='"+file.url+"'  style='"+_width+_height+"border:10px;padding:10px;'></a>";
						showHtml +="</div>"
					...
					showHtmlHead +="<div class='p_container' style='border:10px;padding:10px;'>";
					showHtmlHead += showHtml;
					showHtmlHead +="</div>";
					showHtmlHead +="</td>";
					showHtmlHead +="</tr>";
	...
	for(var i=0;i<files.length;i++){
		...
		if(filee[1]=="jpg"||filee[1]=="gif"||filee[1]=="png"||filee[1]=="bmp"||filee[1]=="tif"){
		 ...
		 //if(filee[1]=="txt"||filee[1]=="doc"||filee[1]=="wps"||filee[1]=="xls"||filee[1]=="pdf"||filee[1]=="ppt"||filee[1]=="docx"){
			var number = 0;
